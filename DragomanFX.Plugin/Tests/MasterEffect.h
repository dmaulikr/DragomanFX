...
|                                GLOBAL PARAMETERS                               |
\*==============================================================================*/
#define MASTEREFFECT_TOGGLEKEY	0x73	/* RANGE[int] | */
...
//CARTOON
#define CartoonPower         		10     //[0.1 to 10.0] Amount of effect you want.
#define CartoonEdgeSlope     		1     //[0.1 to 8.0] Raise this to filter out fainter edges. You might need to increase the power to compensate. Whole numbers are faster.
...
#define DPXColorGamma    		1.5  	//[0.1 to 2.5] Adjusts the colorfulness of the effect in a manner similar to Vibrance. 1.0 is neutral.
#define DPXSaturation 			2  	//[0.0 to 8.0] Adjust saturation of the effect. 1.0 is neutral.
#define DPXRedC   			0.36 	//[0.60 to 0.20] 
#define DPXGreenC 			0.36 	//[0.60 to 0.20]
#define DPXBlueC  			0.34 	//[0.60 to 0.20]
#define DPXBlend 			0.4  	//[0.00 to 1.00] How strong the effect should be.
...
#define Exposure    			-0.15  	//[-1.000 to 1.000] Adjust exposure
#define Saturation  			0.000  	//[-1.000 to 1.000] Adjust saturation
#define Bleach      			0.000  	//[0.000 to 1.000] Brightens the shadows and fades the colors
#define Defog       			0.05  	//[0.000 to 1.000] How much of the color tint to remove
#define FogColor 			float3(0.60, 0.50, 0.55) //[0.00 to 2.55, 0.00 to 2.55, 0.00 to 2.55] What color to remove - default is blue
...
//SKYRIM TONEMAPPING
#define POSTPROCESS			1 	//[1 to 6] Mode of postprocessing you want. Mode 1 uses V1 values, Mode 2 uses V2 values etc
//
...
#define moodR				2.0 	//[0.0 to 2.0] How strong dark red colors shall be boosted
#define moodG				1.1 	//[0.0 to 2.0] How strong dark green colors shall be boosted
#define moodB				0.7 	//[0.0 to 2.0] How strong dark blue colors shall be boosted
...
//FILMICPASS
#define Strenght			0.5  	//[0.05 to 1.5] Strength of the color curve altering
#define BaseGamma			1	//[0.7 to 2.0] Gamma Curve
#define Fade				0.1	//[0.0 to 0.6] Decreases contrast to imitate faded image
#define Contrast			1.0	//[0.5 to 2.0] Contrast.
#define FSaturation			-0.15
#define FBleach				0.8 	//[-0.5 to 1.0] More bleach means more contrasted and less colorful image
...
#define ReinhardLinearSlope		2.3 	//[1.0 to 5.0] how steep the color curve is at linear point. You need color curve understanding to know what this means, just experiment.
...
//SPHERICAL TONEMAP
#define sphericalAmount 		0.05	//[0.0 to 2.0] Amount of spherical tonemapping applied...sort of

//COLOR HUE FX
#define USE_COLORSAT 			1   	//[0 or 1] This will use original color saturation as an added limiter to the strength of the effect
#define hueMid  			0.2	//[0.0 to 1.0] Hue (rotation around the color wheel) of the color which you want to keep
#define hueRange  			0.6	//[0.0 to 1.0] Range of different hue's around the hueMid that will also kept. Using a max range of 1.0 will allow the reverse of the effect where it will only filter a specific hue to B&W
...
#define fAnamFlareWideness		1.2	//[1.0 to 2.5] Horizontal wideness of flare. Don't set too high, otherwise the single samples are visible
#define fAnamFlareAmount		8	//[1.0 to 20.0] Intensity of anamorphic flare.
...
#define fBloomAmount			0.5	//[1.0 to 20.0] Intensity of bloom.
#define fBloomSaturation 		1	//[0.0 to 2.0] Bloom saturation. 0.0 means white bloom, 2.0 means very very colorful bloom.
...
#define ChapFlareTreshold		0.7	//[0.7 to 0.99] Brightness threshold for lensflare generation. Everything brighter than this value gets a flare.
...
#define ChapFlareDispersal 		1	//[0.25 to 1.0] Distance from screen center (and from themselves) the flares are generated. 		
...
#define ChapFlareIntensity 		20	//[5.0 to 20.0] Intensity of flares and halo, remember that higher threshold lowers intensity, you might play with both values to get desired result.
...
#define fGodrayDensity			2.0	//[0.2 to 2.0] Density of rays, higher means more and brighter rays
#define fGodrayThreshold		0.6    	//[0.6 to 1.0] Minimum brightness an object must have to cast godrays
...
#define AO_METHOD			2	//[1 to 2] 1: SSAO | 2: Raymarch AO | 3: HBAO | 4: SSGI | 5: Raymarch HBAO | 6: Alchemy SAO
...
#define AO_BLUR_STEPS			15	//[5 to 15] Offset count for AO smoothening. Higher means more smooth AO but also blurrier AO.
...
#define AO_LUMINANCE_UPPER		0.5	//[0.0 to 1.0] Upper brightness threshold where AO starts to fade out. Above this threshold AO is 0.
#define AO_FADE_START			0.1	//[0.0 to 1.0] Distance from camera where AO starts to fade out. 0.0 means camera itself, 1.0 means infinite distance.
#define AO_FADE_END			0.2	//[0.0 to 1.0] Distance from camera where AO fades out completely. 0.0 means camera itself, 1.0 means infinite distance.

//AMBIENT OCCLUSION - SSAO
#define iSSAOSamples   			32	//[32 to 128] Amount of samples. Don't set too high or shader compilation time goes through the roof.
#define fSSAOSamplingRange		20.0	//[10.0 to 50.0] SSAO sampling range. High range values might need more samples so raise both.
#define fSSAODarkeningAmount 		5	//[0.0 to 5.0] Amount of SSAO corner darkening
#define fSSAOBrighteningAmount  	3.0	//[0.0 to 5.0] Amount of SSAO edge brightening

//AMBIENT OCCLUSION - RAYMARCH SSAO
#define iRayAOSamples   		10	//[10 to 78] Amount of sample "rays" Higher means more accurate AO but also less performance.
#define fRayAOSamplingRange   		0.001	//[0.0001 to 0.02] Range of AO sampling. Higher values ignore small geometry details and shadow more globally.
#define fRayAOMaxDepth  		0.01	//[0.01 to 0.2] Range clip factor to avoid far objects to occlude close objects just because they are besides each other on screen.
#define fRayAOMinDepth 			0.000	//[0.000 to 0.001] Minimum depth difference cutoff to prevent (almost) flat surfaces to occlude themselves.
#define fRayAOPower			1.4	//[0.2 to 5.0] Amount of darkening.

//AMBIENT OCCLUSION - HBAO
#define iHBAOSamples			36	//[7 to 36] Amount of samples. Higher means more accurate AO but also less performance.
...
#define fSSGIIlluminationMult 		8.0	//[1.0 to 8.0] Multiplier of SSGI illumination (color bouncing/reflection).
...
#define DOF_FARBLURCURVE 		2	//[0.4 to X] Elementary, my dear Watson: Blur power of areas behind focus plane.
...
#define DOF_MANUALFOCUSDEPTH 		0	//[0.0 to 1.0] Manual focus depth. 0.0 means camera is focus plane, 1.0 means sky is focus plane.

//DEPTH OF FIELD - RING DOF
#define iRingDOFSamples   		8  	//[5 to 30] Samples on the first ring. The other rings around have more samples
#define iRingDOFRings   		4  	//[1 to 8] Ring count
#define fRingDOFThreshold  		2 	//[0.8 to 2.0] Threshold for bokeh brightening. Above this value, everything gets much much brighter. 1.0 is maximum value for LDR games like GTASA, higher values work only on HDR games like Skyrim etc.
...
#define fMagicDOFColorCurve 		3	//[1.0 to 10.0] DOF weighting curve.

//DEPTH OF FIELD - GP65CJ042 DOF
#define	iGPDOFQuality 			7	//[0 to 7] 0: only slight gaussian farblur but no bokeh. 1-7 bokeh blur, higher means better quality of blur but less fps. 
...
#define fMatsoDOFChromaPow		0.2	//[0.2 to 3.0] Amount of chromatic abberation color shifting.
#define fMatsoDOFBokehCurve		0.5	//[0.5 to 20.0] Bokeh curve.
#define fMatsoDOFBokehLight		0 	//[0.0 to 2.0] Bokeh brightening factor.
...
#define fBorderRatio 1.7777777777777777777						//float(1920.0 / 1080.0)  	//[0.1 to 10.0] Set the desired ratio for the visible area. You MUST use floating point - Integers do not work right.
